...
class UnlabeledMatchingSMR(Function):
    @staticmethod
    def forward(ctx, features, pid_labels, queue, tail, omega):
...
class UnlabeledMatchingLayerSMR(nn.Module):
    """
    ...
    def forward(self, features, pid_labels):
        ...
        """
        scores = UnlabeledMatchingSMR.apply(features, pid_labels, self.queue, self.tail, self.omega)
        return scores